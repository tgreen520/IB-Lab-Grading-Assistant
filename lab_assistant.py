...
- SIGNIFICANT FIGURES: Sig figs must be correct in final reported values.
...
- IMPACT SCORING (Must discuss impact on data collected):
  * Impact defined for 100% of errors = 2 pts.
  * Impact defined for SOME (not all) errors (Partial): -1.0 deduction.
...
5.  **DATA ANALYSIS (Section 7):**
    ...
    * **Significant Figures:** * >3 errors in final reported values? -> -1.0.
...
7.  **EVALUATION (Section 9) - STRICT IMPACT & IMPROVEMENT AUDIT:**
    ...
    * **IMPACT (Must discuss impact on data collected):** * All errors have specific impact defined? -> No deduction.
        * Only some errors have impact defined (Partial)? -> -1.0 deduction.
...
**IMPACT/IMPROVEMENT AUDIT:** * "Did you discuss the impact of errors on the data collected? (No discussion = -2.0. Partial discussion = -1.0)."
...
def grade_submission(file, model_id):
    ...
    if ext == 'docx':
        ...
        prompt_text = (
            ...
            "8. **EVALUATION:** Check if systematic vs random errors are differentiated (-0.5 if not). Check for discussion of IMPACT of errors on data (missing = -2.0, partial = -1.0). Penalize vague improvements. Must specify DIRECTION of error and SPECIFIC equipment for **ALL** errors. (0 pts if missing, 1 pt if partial).\n"
